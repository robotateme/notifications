...
- `event_id` хранится в `inbox_messages` с unique constraint;
...
2. Consumer собирает `IncomingMessage` из `event_id`, topic, key и payload.
...
5. Если сообщение уже `processed`, handler не вызывается.
...
- `processing` - сообщение принято в работу.
...
- повторный `event_id` не вызывает обработчик второй раз;
